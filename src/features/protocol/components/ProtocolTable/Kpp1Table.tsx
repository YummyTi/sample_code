...
const Table: React.FC<ProtocolEditTable> = ({columns, data, setData}) => {
    console.log('KPP1 data: ', data);

    const sumValues = (index: number, id: string, value: number) => {
        console.log('Sum values KPP1: ', data);
...
export const Kpp1Table: React.FC<ProtocolTable> = ({type}) => {
    const {t} = useTranslation();

    const location = useLocation();
    const {state} = location as LocationState;
    const {kpp1Data, setKpp1} = useKPPStore((state) => ({...state}), shallow);
